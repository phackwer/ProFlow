...
class SanSIS_Grid_Table extends Bvb_Grid implements Bvb_Grid_Deploy_Interface {
	const OUTPUT = 'table';
	const MESSAGE_NO_RECORD = 'No records found';

	protected static $_message = array(
		self::MESSAGE_NO_RECORD => SanSIS_Grid::DEFAULT_MESSAGE_NO_RECORD,
	);
	/**
	 ...
	 */
	protected $_hash;
	protected $_gridPagionationDisplay;
	...
	function __construct($options) {
		...
		$this->addTemplateDir('SanSIS/Grid/Template', 'SanSIS_Grid_Template', 'table');
		$this->setDefaultEscapeFunction(false);
	...
	protected function _removeFormParams($post, $extra = array()) {
		...
		if (count($extra) > 0)
			$post = array_merge($post, array_combine($extra, $extra));
	...
	protected function _pagination() {
		...
		if ($this->getInfo("ajax") !== false) {
			$pag = ($actual == 1) ? '<li class="current">1</li>' : "<a href=\"javascript:gridAjax('{$this->getInfo("ajax")}','$url/star{$this->getGridId()}t/0')\">1</a>";
		} else {
			$pag = ($actual == 1) ? '<li class="current">1</li>' : "<li><a href=\"{$url}start{$this->getGridId()}=0\">1</a></li>";
		}
		...
		if ($npaginas > 5) {
			...
			for ($i = $in + 1; $i < $fin; $i++) {
				if ($this->getInfo("ajax") !== false) {
					$pag .= ( $i == $actual) ? " <li class='current'>$i</li>" : " <a href=javascript:gridAjax('{$this->getInfo("ajax")}','$url/start{$this->getGridId()}/" . (($i - 1) * $ppagina) . "')> $i </a>";
				} else {
					$pag .= ( $i == $actual) ? "<li class='current'>$i</li>" : " <li><a href='{$url}start{$this->getGridId()}=" . (($i - 1) * $ppagina) . "'>$i</a></li>";
				}
			}
		...
		if ($this->getInfo("ajax") !== false) {
			$pag .= ( $actual == $npaginas) ? "<li class='current'>" . $npaginas . "</li>" : " <a href=\"javascript:gridAjax('{$this->getInfo("ajax")}','$url/start{$this->getGridId()}/" . (($npaginas - 1) * $ppagina) . "')\">$npaginas</a> ";
		} else {
			$pag .= ( $actual == $npaginas) ? "<li class='current'>" . $npaginas . "</li>" : " <li><a href=\"{$url}start{$this->getGridId()}=" . (($npaginas - 1) * $ppagina) . "\">$npaginas</a> </li>";
		}

		if ($actual != 1) {

			if ($this->getInfo("ajax") !== false) {
				$pag = " <a href=\"javascript:gridAjax('{$this->getInfo("ajax")}','$url/start{$this->getGridId()}/0')\">" . $this->__('Início') . "</a>&nbsp;&nbsp;<a href=\"javascript:agridAjax('{$this->getInfo("ajax")}','$url/start/" . (($actual - 2) * $ppagina) . "')\">" . $this->__('Anterior') . "</a>&nbsp;&nbsp;" . $pag;
			} else {

				$pag = " <li><a href=\"{$url}start=0\">" . $this->__('Início') . "</a>&nbsp;&nbsp;<a href=\"{$url}start{$this->getGridId()}=" . (($actual - 2) * $ppagina) . "\">" . $this->__('&laquo; Anterior') . "</a></li>&nbsp;&nbsp;" . $pag;
			}
		}
		...
		if ($actual != $npaginas) {
			if ($this->getInfo("ajax") !== false) {
				...
				$pag .= "&nbsp;&nbsp;<a href=\"javascript:gridAjax('{$this->getInfo("ajax")}','$url/start{$this->getGridId()}/" . ($actual * $ppagina) . "')\">" . $this->__('Próximo') . "</a> <a href=\"javascript:gridAjax('{$this->getInfo("ajax")}','$url/start{$this->getGridId()}/" . (($npaginas - 1) * $ppagina) . "')\">" . $this->__('Último') . "&nbsp;&nbsp;</a>";
			} else {

				$pag .= "&nbsp;&nbsp;<li><a href=\"{$url}start{$this->getGridId()}=" . ($actual * $ppagina) . "\">" . $this->__('Próximo &raquo;') . "</a>&nbsp;&nbsp;<a href=\"{$url}start{$this->getGridId()}=" . (($npaginas - 1) * $ppagina) . "\">" . $this->__('Último') . "</a></li>";
			}
		}
		...
		if ($npaginas > 1 || count($this->_export) > 0) {
			...
			$this->_render['export'] = $this->_temp['table']->export = $this->getCombo();

			if ((int) $this->getInfo("limit") > 0) {
				$result2 .= str_replace(array('{{pagination}}', '{{numberRecords}}'), array('', (int) $this->getInfo("limit")), $this->pagination());
				...
				$result2 .= str_replace(array('{{pagination}}', '{{numberRecords}}'), array($pag, $this->__('Exibindo') . ' ' . $registoActual . ' ' . $this->__('-') . ' ' . $registoFinal . ' ' . $this->__('de') . '  ' . $this->_totalRecords . ' ' . $this->__('Registro(s)')), $this->pagination());
				...
				$result2 .= str_replace(array('{{pagination}}', '{{numberRecords}}'), array('', $this->__('Exibindo') . ' ' . $registoActual . ' ' . $this->__('-') . ' ' . $this->_totalRecords . ' ' . $this->__('de') . ' ' . $this->_totalRecords . ' ' . $this->__('Registro(s)')), $this->pagination());
				...
				$result2 .= str_replace(array('{{pagination}}', '{{numberRecords}}'), array($pag, $this->_totalRecords), $this->pagination());
			}
		} else {
			return '';
		}
	...
	function deploy() {
		...
		if ((isset($showsForm['form']) && $showsForm['form'] == 1 && $this->getInfo("doubleTables") == 1) || !isset($showsForm['form'])) {
			$this->_render['start'] = $this->globalStart();
		...
		if (((!$this->getParam('edit') || $this->getParam('edit') != 1) && (!$this->getParam('add') || $this->getParam('add') != 1)) || $this->_gridSession->_noForm == 1 || $this->getInfo("doubleTables") == 1) {
			...
			if ($this->_isDetail == true || ($this->_deleteConfirmationPage == true && $this->getParam('gridRemove') == 1)) {
				...
				$this->_render['detail'] = $this->globalStart();
		...
		//$this->_renderDeploy['end'] .= $this->pagination();
		...
		//Apenas adicionar form se existir mainActions
		if ($this->_options['acao']) {
			$hash = $this->getHash();
			$formId = 'formGrid-' . $hash;
			$grid .= "<form id='" . $formId . "' name='" . $formId . "' class='formGrid' action='' onsubmit='this.action = $(\"#formAction-{$hash}\").val();' method='post'>" . PHP_EOL;
			$grid .= "<input name='formAction-{$hash}' id='formAction-{$hash}' type='hidden' />" . PHP_EOL;
		}

		$grid .= "<div class='listagem' id='{$gridId}'>" . PHP_EOL;
		$grid .= implode($this->_renderDeploy) . PHP_EOL;
		$grid .= "</div>" . PHP_EOL;
	...
	public function setPaginationDisplay($flag) {
		$this->_gridPagionationDisplay = $flag;
		return $this;
	}
	...
	public function getPaginationDisplay() {
		return $this->_gridPagionationDisplay;
	}
			...
			$script .= "function _" . $this->getGridId() . "gridChangeFilters(fields,url,Ajax)
{
	...
	//Adicionado novos combos na action no padrão SanSIS
	public function getCombo() {
		$stHtmlCombo = '';

		if ($this->_options['acao']) {
			$arAcao = $this->_options['acao'];
			$stHtmlCombo = '<label>+Ações:</label>
							<select name="acao" class="acao" id="acao">';
			$stHtmlCombo .= '<option value="" selected="selected" disabled="disabled">Selecione ...</option>';
			foreach ($arAcao as $key => $acao) {
				$stHtmlCombo .= '<option value="' . $arAcao[$key]['url'] . '">' . $key . '</option>';
			}
			$stHtmlCombo .='</select>';

			$hash = $this->getHash();

			if (isset($this->_options['acao']['evento']) && $this->_options['acao']['evento']) {
				$stHtmlCombo .='<input type="button" onclick="javascript:' . $this->_options["acao"]["evento"] . '();" value="OK" />';
			} else {
				$stHtmlCombo .='<input type="button" class="button" onclick="if($(\'select#acao\').val() != \'\'){$(\'#formAction-' . $hash . '\').val($(\'select#acao\').val());$(\'#formGrid-' . $hash . '\').submit();}" value="OK" />';
			}
		}
	...
	public function pagination() {
		$st = "</table>" . PHP_EOL;
		$st .= "<div class=\"listagemNavegacao\">" . PHP_EOL;

		$totalRecords = $this->getSource()->getTotalRecords();

		if ($this->_temp['table']->export && $totalRecords > 0) {
			$st .= "<div class=\"listagemAcoes\">" . $this->_temp['table']->export . "</div>" . PHP_EOL;
		}

		if ($this->_pagination > 0 && $totalRecords > 0 && $this->isPaginationEnabled()) {
			$st .= "<div class=\"listagemPaginacao\">\n{{numberRecords}} {{pagination}}\n</div>" . PHP_EOL;
		}
		$st .= "</div>" . PHP_EOL;
		return $st;
	}
	...
	public function globalStart() {
		...
		"\t<caption>" . $this->_options["titulo"] . "</caption>";
	}

	public function titlesStart() {
		return "<thead>\t<tr>" . PHP_EOL;
	}
	...
	public function getMessage($key) {
		if (array_key_exists($key, self::$_message)) {
	...
	protected function __($message) {
		$translate = $this->getMessage($message);
		if ($translate) {
			return $translate;
		} else {
			return parent::__($message);
		}
	}
